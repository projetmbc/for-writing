...
def enregistrer_palettes_similaires(nom_palette, dossier_sortie, n=5, allready_clustered =set()):
    ...
    # Enregistrement
    chemin_fichier = os.path.join(dossier_sortie, f"similaires_{nom_palette}.png")
    plt.savefig(chemin_fichier, dpi=150, bbox_inches='tight')
    plt.close(fig)
...
if __name__ == "__main__":
    ...
    for name in ALL_PALETTES:
        if name in allready_clustered:
            continue

        nb_clusters += 1

        new_cluster = enregistrer_palettes_similaires(name, CLUSTERS_DIR, 10, allready_clustered)
        new_cluster.add(name)

        allready_clustered |= new_cluster

        if True:
            print(", ".join(sorted(new_cluster)))
