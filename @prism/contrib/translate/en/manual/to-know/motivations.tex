...
\begin{tdocnote}
    Using the \LUADRAW\ implementation of \thisproj, see the section \ref{products-luadraw}, we can display the palettes below made from the previous one named \tdoccodein{lua}{'GeoRainbow'}. Each instruction used is given below each palette.

    \centering

    \smallskip
    \subimport{../graphics/}{palette-reversed.luadraw}

    \tdoccodein{lua}{getPal('GeoRainbow', {reverse = true})}

    \smallskip
    \subimport{../graphics/}{palette-shift.luadraw}

    \tdoccodein{lua}{getPal('GeoRainbow', {shift = 3})}

    \smallskip
    \subimport{../graphics/}{palette-extract.luadraw}
