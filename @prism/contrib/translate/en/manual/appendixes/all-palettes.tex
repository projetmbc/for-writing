% !TEX TS-program = lualatex

\documentclass{tutodoc}

\usepackage{../preamble.cfg}

%\geometry{showframe=true}

\begin{document}

\section*{Appendix 1 -- The \palNb\ palettes at a glance} \label{all-palettes}
\addcontentsline{toc}{section}{Appendix 1 -- The \palNb\ palettes at a glance}

The palette names used in this appendix are standard. Depending on the chosen implementation, they may be prefixed.


\begin{tdocimp}
    Categories were generated semi-automatically using a program, followed by manual selection to obtain relevant choices.
    If you identify any errrors, please contact the author of \thisproj.
\end{tdocimp}


% ----------------------------------------- %
% -- JUST TRANSLATE TITLES INSIDE BRACES -- %
% ----------------------------------------- %

\foreach \nb/\title in {%
% AUTO CATEGOS - START
  1/{Colorblind-friendly palettes (coming from Scientific Coulour Maps) -- 40 palettes},%
  2/{Two-color palettes -- 29 palettes},%
  3/{Three-color palettes -- 41 palettes},%
  4/{Rainbow-style palettes -- 41 palettes},%
  5/{High-contrast palettes -- 16 palettes}%
% AUTO CATEGOS - END
}{
  \ifthenelse{\nb = 1}{}{\newpage}

  \subsection*{\title}
  \addcontentsline{toc}{subsection}{\textbullet\,\title}
  %
  \subimport{../../../common/category/}{catego-\nb.luadraw}
}

\end{document}
