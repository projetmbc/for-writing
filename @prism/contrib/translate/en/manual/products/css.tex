...
Each palette color is defined as an individual variable named according to the pattern \tdoccodein{css}+--pal<name>-<nb>+, where \tdoccodein{css}+<name>+ is the standard palette name and \tdoccodein{css}+<nb>+ is the desired index ranging. Each palette color variable is defined as an \tdoccodein{text}+RGB+ value using percentage notation. For instance, the following snippet is an excerpt from \tdoccodein{text}+palettes-hf/Accent.css+.
...
\begin{tdocnote}
For the size-40 \tdoccodein{text}+Accent+ palette, use \tdoccodein{text}+palettes-s40/Accent.css+.
\end{tdocnote}
