...
Accessing a single palette color is straightforward: use \tdoccodein{latex}+\palUse{<name>}{<index>}+ where \tdoccodein{text}+<name>+ is the standard palette name (without prefix), and \tdoccodein{text}+<index>+ is the color number (ranging from 1 to 10). For example, \tdoccodein{latex}+\palUse{GistHeat}{8}+ is the eighth color of the \tdoccodein{text}+GistHeat+ palette, an {\xcolor} format color that can be easily used as shown in the following compilable example.
...
\begin{enumerate}
\item
\tdoccodein{latex}+\palCreateFromNames+ works with a comma separated list of named colors, while \tdoccodein{latex}+\palCreateFromRGB+ creates a palette by entering it as an array-like variable.
