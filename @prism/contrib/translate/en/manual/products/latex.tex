...
\begin{tdocnote}
For the size-40 \tdoccodein{text}+Accent+ palette, use \tdoccodein{text}+\usepackage{palettes-s40/Accent}+.
\end{tdocnote}

\subsubsection{Creating palettes from scratch}

Internally, palettes are defined using an array-of-three-floats syntax. For instance, the following snippet is an excerpt from \tdoccodein{text}+palettes-hf/Accent.sty+.
...
The internally stored definition of a palette named \tdoccodein{text}+MyPal+, for example, is \tdoccodein{latex}+\g_palette_MyPal_seq+ which is a {\LaTeX3} variable (keep in mind the pattern \tdoccodein{text}+\g_palette_<palette-name>_seq+).

\begin{tdocnote}
Variables of type \tdoccodein{text}+\g_palette_<palette-name>_seq+ are not used internally to retrieve the colors themselves; they are only there for technical reasons related to the development process of new palettes via {\LaTeX}.
\end{tdocnote}
