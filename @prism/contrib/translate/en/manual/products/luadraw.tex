...
You can use {\thisproj} palettes with {\LUADRAW} which is a package that greatly facilitates the creation of high-quality 2D and 3D plots via {\LuaLaTeX} and {\TIKZ}.
...
\item
For compatibility reasons with the {\LUADRAW} API, there is also an associative array called \tdoccodein{lua}+palNames+, which expects the variable name with \tdoccodein{text}+pal+ prefix. See the caution note at the end of this section.
\end{itemize}
...
To simplify the explanations, we will refer to the colors in the standard palette \tdoccodein{text}+'GistHeat'+ as \tdoccodein{lua}+coul_1+, \tdoccodein{text}+coul_2,+, etc. The options are then \textbf{processed in the following order}.
...
\begin{tdoccaut}
The current version of {\LUADRAW} simply uses the palettes provided by {\thisproj} wihtout the \tdoccodein{lua}+getPal+ function. If you prefer to use the {\thisproj} version with its \tdoccodein{lua}+getPal+ function, you will need to include the entire code in the \tdoccodein{text}+luadraw_palettes.lua+ file where the package is installed.
\end{tdoccaut}
