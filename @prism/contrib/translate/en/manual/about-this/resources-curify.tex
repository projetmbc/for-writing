...
\begin{itemize}
    ...
    \thisproj\ maintains a one-to-one mapping%
    \footnote{
        Some \matplotlib\ palettes are duplicated, likely for historical reasons.
    }
    between names and palettes.%
    \footnote{
        This does not exclude visually similar palettes with distinct names, which are preserved to honor specific design intents.
        However, if you find palettes with indistinguishable color sets, at least to the naked eye, please report this as a bug.
    }

    \item \textbf{No basic derived versions.}
    \thisproj\ omits palettes that can be reconstructed by shifting%
    \footnote{
    ...
    existing ones, because most \thisproj\ implementations offer an API to handle these transformations dynamically.


    \item \textbf{Reasonable palette sizes.} 
    Palettes exceeding \palMaxSize\ colors are ignored%
    \footnote{
        \cmasher\ offers the \tdoccodein{text}+neutral+ palette, which contains 2560 colors
    }. 
