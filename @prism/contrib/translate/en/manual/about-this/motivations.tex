...
\begin{tdocnote}
    ...
    \begin{center}
        ...
        \tdoccodein{lua}+palCreateFromPal('GeoRainbow', {reverse = true})+

        \smallskip
        \includegraphics{../../../common/graphics/palette-shift.pdf}

        \tdoccodein{lua}+palCreateFromPal('GeoRainbow', {shift = 3})+

        \smallskip
        \includegraphics{../../../common/graphics/palette-extract.pdf}

        \tdoccodein{lua}+palCreateFromPal('GeoRainbow', {extract = {7, 1, 4}})+
    \end{center}

    \smallskip

    This features provide remarkable creative flexibility: with the same surface as before, using the setting
    \tdoccodein{lua}+palCreateFromPal('GeoRainbow', {extract = {6, 5, 4, 3, 1, 2}})+
    instead of
    \tdoccodein{lua}+palCreateFromPal('GeoRainbow')+,
